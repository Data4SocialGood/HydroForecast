...
export const Chart = (props: IChartProps) => {
  const {dataset, series, otherSeries, aggType, measureCol, dataSource} = props;
  ...
  if (chartType === 'heatmap') {
    ...
    if(measure === 0) {
      seriesData = otherSeries.map(groupedStat => ([xCategories.findIndex(c => c === groupedStat.group[0]), yCategories.findIndex(c => c === groupedStat.group[1]), groupedStat.value]));
    }
    else {
    ...
    if(measure === 0) {
      seriesData = otherSeries.map(groupedStat => ({
  ...
  const handleMeasureChange = (e, {value}) => {
    setMeasure(value);
    props.updateMeasure(dataset.id, value);
  }
  ...
  const yAxis = groupByCols && groupByCols.length > 1 ? dataset.dimensions.find(d => d === groupByCols[1]) : dataset.dimensions.find(d => d === xAxisOptions[1].key);
  ...
  return <Segment id='chart-container' raised textAlign='center'>
    ...
    <HighchartsReact
      ...
      options={chartType === 'heatmap' ? {
        ...
        tooltip: {
          formatter() {
            let s = "<b>" + seriesData[this.point.x].name + "</b>: ";
            s +=  this.point.value != null ? this.point.value.toFixed(2) : this.point.y.toFixed(2);
            return s;
          }
        },
        colorAxis: null,
      ...
      <Label.Group>
        ...
        <span> of <Dropdown
          ...
          options={[
            ...
            {text: dataset.headers[dataset.measure0], value: dataset.measure0},
            {text: dataset.headers[dataset.measure1],  value: dataset.measure1 }]}
